...
class VectorStore:
    def __init__(self, 
        ...
        # Initialize ChromaDB client (updated API)
...
def load_and_index_embeddings(embeddings_dir: str = "models/embeddings",
    ...
    """
    Load embeddings and index them in vector store
    ...
    if vector_store.collection.count() > 0:
        logger.warning(f"⚠️ Collection already has {vector_store.collection.count()} documents")
...
if __name__ == "__main__":
    ...
    # Load an embedding to test query
    embeddings = np.load("models/embeddings/embeddings.npy")
    test_embedding = embeddings[0]  # Use first embedding as test
